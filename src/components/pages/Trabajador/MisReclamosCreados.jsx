...
import style from './MisReclamosCreados.module.css';

const MisReclamosCreados = () => {
      ...
      <div className={style.reclamosList}>
        {reclamos.length === 0 ? (
          <div className={style.sinReclamos}>
            <p>No has creado ningún reclamo</p>
          </div>
        ) : (
            ...
            <div key={reclamo.id_reclamo} className={style.reclamoCard}>
              ...
              <div className={style.reclamoInfo}>
                <div className={style.infoRow}>
                  <span className={style.label}>Técnico:</span>
                  <span className={style.value}>{reclamo.tecnico_nombre}</span>
                </div>
                <div className={style.infoRow}>
              ...
              {reclamo.estado === 'resuelto' && reclamo.fecha_resolucion && (
                <div className={style.resolucion}>
                  <p className={style.resolucionLabel}>✅ Resuelto el: {formatearFecha(reclamo.fecha_resolucion)}</p>
                </div>
              )}

              <button 
                className={style.btnVerTicket}
                onClick={() => navigate(`/trabajador/tickets/${reclamo.ticket_id_value}`)}
              >
                Ver Ticket
              </button>
            </div>
...
export default MisReclamosCreados;