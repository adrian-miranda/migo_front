 ...
 * Muestra el historial completo de tickets atendidos por el técnico
 */
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import style from './TecnicoDetail.module.css';

const TecnicoDetail = ({ tecnico, onClose }) => {
  const navigate = useNavigate();
  const [filtroEstado, setFiltroEstado] = useState('todos');
  const [filtroTipo, setFiltroTipo] = useState('todos');

  // Filtrar tickets
  const ticketsFiltrados = tecnico.tickets.filter(ticket => {
  ...
  const estadosUnicos = [...new Set(tecnico.tickets.map(t => t.estado))];
  
  // Obtener tipos únicos
  const tiposUnicos = [...new Set(tecnico.tickets.map(t => t.categoria))];

  // Estadísticas del técnico
  const totalTickets = tecnico.tickets.length;
  const ticketsPorEstado = {
    abiertos: tecnico.tickets.filter(t => t.estado === 'Abierto').length,
    enProceso: tecnico.tickets.filter(t => t.estado === 'En Proceso').length,
    resueltos: tecnico.tickets.filter(t => t.estado === 'Resuelto').length,
    cerrados: tecnico.tickets.filter(t => t.estado === 'Cerrado').length,
  };

  const ticketsPorTipo = {};
  tiposUnicos.forEach(tipo => {
    ticketsPorTipo[tipo] = tecnico.tickets.filter(t => t.categoria === tipo).length;
  });

  // Calcular tickets por prioridad
  const ticketsPorPrioridad = {
    baja: tecnico.tickets.filter(t => t.prioridad === 'Baja').length,
    media: tecnico.tickets.filter(t => t.prioridad === 'Media').length,
    alta: tecnico.tickets.filter(t => t.prioridad === 'Alta').length,
    urgente: tecnico.tickets.filter(t => t.prioridad === 'Urgente').length,
  };
    ...
    <div className={style.overlay} onClick={onClose}>
      <div className={style.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={style.header}>
          <div>
            <h2>{tecnico.nombre_completo || 'Técnico'}</h2>
            <p className={style.correo}>{tecnico.correo}</p>
          </div>
        ...
        <div className={style.infoTecnico}>
          <div className={style.infoItem}>
            <span className={style.label}>Estado:</span>
            <span className={`${style.estadoBadge} ${
          ...
          <div className={style.infoItem}>
            <span className={style.label}>Tickets activos:</span>
            <span className={style.value}>{tecnico.tickets_activos || 0}</span>
          </div>
          <div className={style.infoItem}>
          ...
          <div className={style.infoItem}>
            <span className={style.label}>Tiempo promedio:</span>
            <span className={style.value}>
              {tecnico.metricas?.tiempoPromedio !== 'N/A' 
                ? `${tecnico.metricas?.tiempoPromedio}h` 
                : 'N/A'}
            </span>
          </div>
        </div>
        ...
        <div className={style.estadisticas}>
          ...
          {/* Por estado */}
          <div className={style.seccionStats}>
          ...
          <div className={style.seccionStats}>
            ...
            <div className={style.prioridadGrid}>
              <div className={style.prioridadItem}>
                <span className={style.prioridadLabel}>Baja:</span>
                <span className={style.prioridadValor}>{ticketsPorPrioridad.baja}</span>
              </div>
              <div className={style.prioridadItem}>
                <span className={style.prioridadLabel}>Media:</span>
                <span className={style.prioridadValor}>{ticketsPorPrioridad.media}</span>
              </div>
              <div className={style.prioridadItem}>
                <span className={style.prioridadLabel}>Alta:</span>
                <span className={style.prioridadValor}>{ticketsPorPrioridad.alta}</span>
              </div>
              <div className={style.prioridadItem}>
                <span className={style.prioridadLabel}>Urgente:</span>
                <span className={style.prioridadValor}>{ticketsPorPrioridad.urgente}</span>
              </div>
            </div>
          </div>
        ...
        <div className={style.historialContainer}>
          <h3>Historial de Tickets Atendidos ({ticketsFiltrados.length})</h3>
                ...
                <div key={ticket.id_ticket} className={style.ticketCard}>
                  ...
                  <div className={style.ticketBody}>
                    ...
                    <div className={style.ticketInfo}>
                      ...
                      <div className={style.ticketInfoItem}>
                        <span className={style.ticketLabel}>Creado por:</span>
                        <span>{ticket.usuario_creador?.nombre || 'N/A'}</span>
                      </div>
                    </div>

                    <div className={style.ticketFooter}>
                      <span className={style.ticketFecha}>
                        Asignado: {formatearFecha(ticket.fecha_asignacion)}
                      </span>
